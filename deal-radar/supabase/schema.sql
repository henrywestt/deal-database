-- Deal radar schema
-- Self-contained. Everything is prefixed so it lifts out of a larger app cleanly.
...
create type deals_rights_type as enum (
  'naming_rights', 'principal_partner', 'major_partner', 'official_partner',
  'supplier', 'media_rights', 'athlete_endorsement', 'kit_apparel', 'other'
);

create type deals_value_confidence as enum (
  'confirmed', 'estimated', 'undisclosed'
);
...
create table deals_brands (
  ...
  parent_company text,
  category text,
  profile_score smallint not null default 50,
  profile_source text not null default 'default',
  notes text,
  created_at timestamptz not null default now()
);
...
comment on column deals_properties.tier is
  '1 = global or national marquee. 2 = national governing body or top-flight league. '
  '3 = club, state body or mid-tier event. 4 = regional or community. 5 = niche.';
...
-- ---------------------------------------------------------------
-- Scoring config. Weights live here so retuning is a row edit.
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- Row level security. Reads open to anon, writes service role only.
-- ---------------------------------------------------------------
...
alter table deals_deal_articles enable row level security;
alter table deals_sources enable row level security;

create policy deals_read on deals for select to anon, authenticated using (true);
create policy deals_brands_read on deals_brands for select to anon, authenticated using (true);
create policy deals_properties_read on deals_properties for select to anon, authenticated using (true);
create policy deals_articles_read on deals_articles for select to anon, authenticated using (true);
create policy deals_link_read on deals_deal_articles for select to anon, authenticated using (true);
create policy deals_sources_read on deals_sources for select to anon, authenticated using (true);

grant select on deals_ranked to anon, authenticated;
